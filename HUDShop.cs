    ...
    class HUDShop
    {
        ...
        public void UpdateView()
        {
            if (ActiveEq == 1)
            {
                for (int i = ViewIndex2; i < ViewIndex2 + SlotsCount; i++)
                    if (i < Shop.Items.Keys.Count)
                        Slots2[i - ViewIndex2].SetItem(Shop.Items.Keys.ElementAt(i));
                    else
            ...
            GoldLabel.Caption = "Twoje zloto: " + Character.Profile.Gold;
        }
        ...
        public void UpdateDescription()
        {
            ...
            else if (ActiveEq == 1)
            {
                ...
                {
                    DescriptionLabel.Caption =
                        Shop.Items.Keys.ElementAt(SelectIndex2).DisplayName
                        + "\n\n"
                        + Shop.Items.Keys.ElementAt(SelectIndex2).Description;

                    if (Shop.Items.Keys.ElementAt(SelectIndex2) is ItemSword)
                        DescriptionLabel.Caption += "\nObrazenia: "
                            + (Shop.Items.Keys.ElementAt(SelectIndex2) as ItemSword).Damage.ToString();

                    SelectedPicture.Panel.MaterialName = Shop.Items.Keys.ElementAt(SelectIndex2).InventoryPictureMaterial;
                }
                else
                {
                    DescriptionLabel.Caption = "";
        ...
        public int SelectIndex2
        {
            get
            {
                return (_SelectIndex2 >= Shop.Items.Keys.Count ? -1 : _SelectIndex2);
            }
            set
            {
                if (value < 0)
                    value = -1;
                else if (value >= Shop.Items.Keys.Count)
                    value = Shop.Items.Keys.Count - 1;
                _SelectIndex2 = value;
